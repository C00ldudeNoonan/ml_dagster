...
def check_mnist_data_completeness(
...
def check_accuracy_reasonable(
    context, model_evaluation: Dict[str, Any]
) -> dg.AssetCheckResult:
...
def check_production_model_quality(
    context, production_digit_classifier
) -> dg.AssetCheckResult:
    """Check that production model meets deployment criteria (blocking check)."""
    if production_digit_classifier is None:
        return dg.AssetCheckResult(
            passed=False, metadata={"error": "Production model is None"}
        )
...
@dg.multi_asset_check(
    specs=[
        dg.AssetCheckSpec(
            name="check_prediction_format", asset="batch_digit_predictions"
        ),
        dg.AssetCheckSpec(
            name="check_confidence_scores", asset="batch_digit_predictions"
        ),
    ]
)
